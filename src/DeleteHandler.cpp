...
Response	DeleteHandler::handle(const Request &request,
		...
		const ServerConfig &server)
{
	...
	if (!route.uploadEnable)
	{
		return (ErrorResponseHandler::build(
			403,
			"Forbidden: Delete is disabled for this route",
			server));
	}
	...
	if (fullPath.empty())
	{
		return (ErrorResponseHandler::build(
			403,
			"Forbidden: Invalid path sequence",
			server));
	}
	...
	if (S_ISDIR(pathStat.st_mode))
	{
		return (ErrorResponseHandler::build(
			403,
			"Forbidden: Cannot delete a directory",
			server));
	}
	...
	if (std::remove(fullPath.c_str()) != 0)
	{
		return (ErrorResponseHandler::build(
			500,
			"Internal Server Error: Failed to delete the file",
			server));
	}
