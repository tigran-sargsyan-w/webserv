...
namespace
{
	...
	ReadyFdHandler::Result handleCgiFd(const pollfd &pollFd,
		...
		CgiManager &cgiManager, PollManager &pollManager)
	{
		if (cgiManager.handleEvent(pollFd.fd, pollFd.revents, clients,
			...
			return (ReadyFdHandler::ADVANCE_INDEX);
		return (ReadyFdHandler::KEEP_INDEX);
	}

	ReadyFdHandler::Result handleListenerFd(const pollfd &pollFd,
		std::map<int, Client> &clients,
	...
	ReadyFdHandler::Result handleClientFd(const pollfd &pollFd,
		std::map<int, Client> &clients,
...
ReadyFdHandler::Result ReadyFdHandler::handle(const pollfd &pollFd,
	std::map<int, Client> &clients,
	const std::vector<ServerConfig> &configs, CgiManager &cgiManager,
	ListenerSocketHandler &listenerSocketHandler, PollManager &pollManager)
{
