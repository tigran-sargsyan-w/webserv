...
bool SessionHandler::canHandle(const Request &request)
{
	std::string path = normalizePath(request.getPath());

	return (path == "/session" || path == "/logout");
}
...
Response SessionHandler::handle(const Request &request, const ServerConfig &server)
{
...
Response SessionHandler::handleSession(const Request &request)
{
	...
	response.setBody(buildSessionPage(session, created));
...
Response SessionHandler::handleLogout(const Request &request)
{
	...
	response.setStatusCode(200);
	response.setBody(buildLogoutPage(destroyed));
	response.addHeader("Content-Type", "text/html");
	response.addHeader("Set-Cookie", SessionManager::buildExpiredCookieHeader());
	return (response);
}

std::string SessionHandler::buildSessionPage(const SessionData &session, bool created)
{
	...
	body << "</ul>\n";
	body << "<p><a href=\"/session\">Refresh session</a></p>\n";
	body << "<p><a href=\"/logout\">Destroy session</a></p>\n";
	body << "</body></html>\n";
	return (body.str());
}

std::string SessionHandler::buildLogoutPage(bool destroyed)
{
	std::ostringstream body;
	...
	body << "<p><a href=\"/session\">Create a new session</a></p>\n";
	body << "</body></html>\n";
	return (body.str());
}
