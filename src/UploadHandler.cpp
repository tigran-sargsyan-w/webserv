...
static Response	writeUploadedFile(const Request &request,
		...
		const ServerConfig &server)
{
	...
	if (!file.is_open())
	{
		return (ErrorResponseHandler::build(
			500,
			"Internal Server Error: Could not open file",
			server));
	}
	...
	if (file.fail())
	{
		...
		return (ErrorResponseHandler::build(
			500,
			"Internal Server Error: Write failed",
			server));
	}
		...
		const ServerConfig &server)
{
	...
	if (!route.uploadEnable)
	{
		return (ErrorResponseHandler::build(
			403,
			"Forbidden: Upload is disabled for this route",
			server));
	}
	...
	if (route.uploadStore.empty())
	{
		return (ErrorResponseHandler::build(
			500,
			"Internal Server Error: Upload store not configured",
			server));
	}
		...
		&& S_ISDIR(pathStat.st_mode))
	{
		return (ErrorResponseHandler::build(
			409,
			"Conflict: A directory with this name already exists",
			server));
	}
